"""Phase 5 端到端测试:PaddleOCR-VL 1.6 → Markdown → EPUB → 验证。

用法:
    python scripts/test_phase5_e2e.py [pdf 路径]
默认测试 samples/pdfs/扫描版测试.pdf。
需要环境变量 PADDLEOCR_API_KEY / PADDLEOCR_SECRET_KEY。
"""
...
def main() -> int:
    ...
    print(f"===== Phase 5: {name} (PaddleOCR-VL 1.6) =====")
    backend = get_backend("paddleocr")  # 从环境变量读凭证
    ...
    # 3. 验证(期望 ≥1 张图;PaddleOCR 对扫描版实际识别 1 张正文图)
    res = run([sys.executable, str(ROOT / "scripts" / "verify_epub.py"), str(epub), "--expect-images", "1"])
    print(res.stdout)
