...
- **诗行保护**：两行都短（≤ `VERSE_MAX_CHARS`）**且都没有句末标点**时，视为诗行 / 自由分行，不拼接；连续 ≥ 2 行都 ≤ `VERSE_HARD_MAX_CHARS` 时在行尾补 **Markdown 硬换行**（两个空格）。两步都需要：不拼只保住 `book.md` 的换行，pandoc 仍会把换行渲染成空格，**硬换行才让诗在阅读器里真的分行**（引用块里的诗同样处理）。标点条件是必需的 —— 只看长度会误伤「短行散文断行」（两行都短，但下一行以句号结尾，那是正常断行）。
